...
| этап | содержание |
|---|---|
| Ф0 | инвентаризация, отчёт без действий; очистка регенерируемых данных (§10, ≥92 GiB) — заодно обкатка обхода, карантина, журнала и отката |
| Ф1 | провенанс из метаданных, семейства и роли, T0/T1 копии |
| Ф2 | перцептивный слой: pHash + SSIM (T2), эмбеддинги + ORB (T3) |
| Ф3 | серии и выбор лучшего кадра |
| Ф4 | zero-shot категоризатор |
| Ф5 | реорганизация структуры |
| Ф6 | инкрементальный режим и ingest |
| Ф7 | видео (755 GiB — крупнейший потенциал по месту) |

Реорганизация выполняется строго после дедупа, отдельной транзакцией.
Схема `YYYY/YYYY-MM-DD_<событие>/`, событие — временной кластер с разрывом
более 6 часов, опционально GPS и метка CLIP. Полный маппинг путей в журнале.

Определение даты, по убыванию доверия: `DateTimeOriginal` → `CreateDate` →
дата из имени файла → дата из пути → `mtime`. Источник показывается
пользователю, неуверенные выделяются.
...
реорганизация обязана разводить коллизии.
