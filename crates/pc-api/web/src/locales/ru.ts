...
export const ui = {
  ...
  roleHelp: {
    ...
    copy: "Та же картинка, что и у хранимого файла: совпал весь кадр в цвете и в своём разрешении. Для raw-файлов сверяются сами байты. Единственное, что переносится по умолчанию.",
    unknown:
  ...
  subtitles: {
    ...
    tree: "Обойдите папки как в проводнике и скажите, где лежат оригиналы.",
    families:
  ...
  tree: {
    ...
    disks: "Диски архива",
    disksHelp:
      "Корней несколько, а структура одна: дерево ниже сложено по всем дискам сразу. Папка, которая есть на трёх дисках, здесь один узел — и отмечается один раз.",
    everyRoot: "на всех дисках",
    oneDisk: "один диск",
    perDisk: "Эта папка по дискам",
    perDiskHelp:
      "Если оригиналы только на одном диске, а на остальных копии — отметьте здесь именно его. Такая отметка живёт отдельно от общей и на другие диски не распространяется.",
    onlyThisDisk: "Только этот диск",
    mark: "Здесь оригиналы",
    markHelp:
      "Всё, что лежит в этой папке и в её подпапках на любом уровне, считается оригиналом: в каждой группе остаётся файл отсюда. Отметка ставится на путь относительно корня, поэтому накрывает эту папку сразу на всех дисках — и на том, который вы добавите в массив завтра. Она сохраняется и применяется заново после каждой пересборки.",
    unmark: "Снять отметку",
    marks: "Папки с оригиналами",
    noMarks:
      "Ни одна папка ещё не отмечена. Откройте ту, где лежат сами снимки, и нажмите «Здесь оригиналы».",
    folders: "Папки",
    here: "Оригиналы здесь",
    noFolders: "Вложенных папок нет",
    noFiles: "В этой папке нет проиндексированных файлов",
    fromIndex:
      "Дерево построено по индексу: здесь только то, что было просканировано. Файлы на диске не трогаются — отметка меняет лишь то, какую версию снимка архив считает главной.",
    original: "оригинал",
  },
  ...
  protectedSeries:
    "Похоже на pixel-shift, поэтому прореживание недоступно: если это он, все кадры составляют один снимок, и любой потерянный — потерянный снимок. Основание:",
  seriesNote:
    "Ни один кадр серии не становится кандидатом на удаление только из-за участия в серии.",
  reconcile: "Сверить с диском",
  reconcileHelp:
    "Прочитаем манифест записи и посмотрим, где каждый файл лежит на самом деле. Те, что уехали в карантин и не вернулись, вернём; если файл окажется и там и там — решать вам, мы ничего не тронем.",
  pendingNote:
  ...
  // The desktop window only: its data folder and native dialogs.
  desktop: {
    pickTitle: "Выберите папку",
    chooseTitle: "Новая папка данных программы",
    title: "Папка данных программы",
    modes: {
      system: "Системная папка программы в профиле пользователя.",
      custom: "Папка, выбранная вами.",
      portable:
        "Переносной режим: данные рядом с программой (файл photo-cleanup.portable).",
      override: "Папка задана при запуске параметром --data-dir.",
    },
    folder: "Папка",
    database: "База данных",
    thumbs: "Кэш превью",
    size: "База {0}, превью {1} ({2} файлов).",
    sizeUnknown: "Размер не удалось посчитать.",
    free: "Свободно на диске: {0}.",
    reveal: "Открыть папку",
    change: "Сменить папку…",
    fixed:
      "Папку нельзя сменить из окна: она задана переносным режимом или параметром запуска.",
    legacy:
      "Рядом с программой найдена база прежней версии: {0}. Она не подхватывается сама.",
    useSystem: "Перейти в системную папку…",
    legacyAction: "Перейти на неё…",
    photosUntouched:
      "Здесь только база и кэш превью программы. Фотографии и карантин не переносятся и не меняются.",
    previewTitle: "Сменить папку данных",
    from: "Сейчас",
    to: "Новая папка",
    toCopy: "Будет скопировано",
    space: "Место",
    spaceValue: "нужно {0}, свободно {1}",
    howItGoes:
      "Работа остановится, база и превью будут скопированы и проверены, затем программа перезапустится с новой папкой. Старая папка остаётся на месте и ничего из неё не удаляется — удалить её можно самостоятельно, когда убедитесь, что всё на месте. Если новая папка не запустится, экран ошибки предложит вернуться к прежней.",
    working: "Копирование и проверка… Не закрывайте окно.",
    failed: "Смена папки не удалась. Исходные данные сохранены: {0}",
    copy: "Скопировать и перезапустить",
    useExisting: "Перейти на базу в этой папке без копирования",
  },
} as const;

// Screen text and parameterized messages. Keep all translations in this file.
export const messages = {
  plan_combined_title: "Общий план очистки",
  plan_combined_help:
    "Автоматический отбор и ваши решения объединены в одном плане. Выбранные оригиналы и правила папок определяют, что сохранить; отклонённые вручную файлы добавляются, если проходят проверки. Оставленные и отложенные файлы не переносятся. По умолчанию автоматика добавляет только точные копии.",
  plan_scope: "Область плана",
  plan_all: "Весь архив",
  plan_only_reviewed: "Только мои решения",
  plan_only_originals: "Только по отмеченным оригиналам",
  plan_show_all: "Показать общий план",
  plan_scope_help:
    "Ограниченные области — части общего плана, а не отдельные партии для запуска по очереди. Каждый файл учитывается один раз. Подтверждение выполняет выбранную область плана.",
  plan_decisions: "Сохранённые решения и правила папок",
  plan_advanced: "Настройки автоматического отбора",

  tree_next_step: "Дальше — проверить копии",
  tree_plan_help:
    "Здесь отметьте папки с оригиналами. На странице «План и перенос» проверьте, какие копии можно перенести, какие файлы останутся и почему. Сам перенос потребует отдельного подтверждения там.",
  tree_open_plan: "Посмотреть план по оригиналам",
  tree_plan_scope:
    "Отмечено папок: {0}. План учитывает их все по всему архиву, независимо от открытой выше папки.",
  tree_plan_empty:
    "Сначала откройте папку с оригиналами и нажмите «Здесь оригиналы».",
  plan_originals_help:
    "Эта часть общего плана относится к группам, чей сохраняемый файл лежит в отмеченной папке с оригиналами. Ваши ручные решения также учитываются. Копии могут находиться в любой части архива.",
  plan_edit_originals: "Выбрать папки с оригиналами",
  ...
  rq_defer: "Отложить",
  rq_undo: "Отменить решение",
  rq_undone: "Решение отменено. Группа возвращена в очередь.",
  rq_saved: "Решение сохранено. Открыта следующая доступная группа.",
  rq_not_eligible:
    "Быстрое добавление недоступно: есть другие версии или ограничения. Можно оставить всё, отложить или открыть подробный разбор.",
  rq_exact: "Точные копии",
  rq_versions: "Другие версии",
  rq_all: "Все группы",
  rq_pending: "Без решения",
  rq_planned: "В плане",
  rq_kept: "Оставлены",
  ...
  rq_open_plan: "Открыть общий план",
  rq_batch: "Точные копии пакетом",
  rq_batch_scope:
    "Все ещё не разобранные группы точных копий во всём архиве. Фильтры очереди не ограничивают этот пакет.",
  rq_batch_help:
    "Уже принятые и отложенные решения исключены. Добавляются только группы, прошедшие проверки ядра. Файлы не перемещаются.",
  rq_batch_confirm: "Добавить пакет в план",
  rq_progress: "Без решения: {0} · отложено: {1}",
  rq_detailed: "Подробный разбор",
  rq_back: "Вернуться в очередь",
  rq_empty: "В этой очереди нет групп",
  rq_empty_hint:
    "Измените фильтры, вернитесь к отложенным группам или постройте группы после индексации.",
  rq_evidence: "Проверка группы",
  rq_verified: "Совпадение подтверждено",
  rq_needs_review: "Нужно ваше решение",
  rq_exact_help:
    "Ядро подтвердило точные копии сохраняемого файла и допустимость плана. Перед переносом файлы будут перечитаны и проверены снова.",
  rq_after: "После сохранения решения открывается следующая группа.",
  rq_no_move:
    "Сначала решения. Перенос в карантин — отдельно, после проверки плана.",
  rq_zoom: "Масштаб сравнения ×2",
  rq_shortcuts_hint:
    "Клавиши работают вне полей ввода и диалогов. Удержание клавиши не принимает несколько решений.",
  rq_candidate: "Кандидат в карантин",
  rq_compare_version: "Версия для сравнения",
  rq_metadata: "Метаданные и пути файлов",
  rq_manual_choices:
    "В группе уже есть ручные отклонения файлов. Проверьте их в подробном разборе.",
  rq_reviewed_help:
  ...
  pe_companion_reason: "Переносится вместе со снимком {0}.",
  pe_image_error: "Не удалось загрузить снимок",
  pe_title: "Проверка плана",
  pe_search: "Найти фото или папку",
  pe_search_hint: "Имя, часть пути или несколько слов · допускаются опечатки",
  pe_outcome: "Что произойдёт с файлами",
  pe_all: "Все результаты",
  pe_move: "В карантин",
  pe_stay: "Остаётся",
  pe_refusal: "Отказ · остаётся",
  pe_scope:
    "Поиск проверяет весь показанный план. Фильтры меняют только просмотр: подтверждение выполняет весь план.",
  pe_folders: "Папки",
  pe_all_folders: "Все папки",
  pe_up: "На уровень выше",
  pe_folder_scope:
    "Только папки, представленные в этом плане, включая вложенные. Пути указывают фактический диск.",
  pe_results: "Результатов: {0}",
  pe_results_list: "Файлы и отказы плана",
  pe_no_results:
    "В этом плане совпадений нет. Это не полный архив. Проверьте фильтры или откройте дерево архива.",
  pe_prev: "Предыдущая страница",
  pe_next: "Следующая страница",
  pe_detail: "Что произойдёт с файлом",
  pe_open: "Открыть снимок: {0}",
  pe_full: "Открыть крупно",
  pe_source: "Сейчас",
  pe_destination: "После переноса",
  pe_size: "Размер",
  pe_reason: "Почему",
  pe_kept_reason:
    "Этот файл остаётся в архиве и служит сохраняемой версией для копий в плане.",
  pe_select: "Выберите файл, чтобы увидеть результат.",

  otmecheno_grupp:
    "Отмечено. Групп с файлом отсюда: {0}; хранимый файл сменился у {1}.",
  otmetka_snyata: "Отметка снята.",
  drugoy_kadr_a_ne_kopiya:
    "В {0} группах эта папка держит другой кадр, а не копию хранимого файла. Их не трогаем: такое решает человек — на странице «Дубликаты и версии».",
  pokazany_pervye_iz: "Показаны первые {0} из {1}.",
  vnutri_otmechennoy_papki: "Внутри отмеченной папки {0}",
  versiy_v_gruppe: "версий: {0}",
